		...
		<view class="login-form">
			<form @submit="formSubmit">
				<view class="form-input">
					<input type="text" v-model="name" placeholder="请输入注册的用户名" />
				</view>
				<view class="form-input">
					<input type="text" v-model="tel" placeholder="请输入注册的手机号" />
				</view>
				<view class="form-input">
					<input type="password" v-model="password" placeholder="请输入注册密码确保3~16位" @focus="passwordF_B" @blur="passwordF_B" />
				</view>
				<view class="form-input">
					<input type="password" v-model="passwquary" placeholder="请再次确认密码" @focus="passwordF_B" @blur="passwordF_B" />
				</view>
				<button v-on:click="goto()" type="primary" form-type="submit">注册</button>
			</form>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			goto(){
				...
				if (this.password<3||this.password>16){
					if (this.name.length <= 0) {
						uni.showToast({
						title: '请确保您输入的密码符合规格',
					    icon: 'none'
				...
				if (this.password!=this.passwquary){
					uni.showToast({
					title: '请再次确认密码是否一致',
					icon: 'none'
					});
				...
				wx.request({
					url:'http://192.168.137.218:8081/register',
					data:{
						name:this.name,
						tel:this.tel,
