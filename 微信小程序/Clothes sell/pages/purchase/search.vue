<template>
	<view class="content">
		<view class="search-box">
			<mehaotian-search-revision class="mSearch-input-box" v-model="keyword"
				:mode="2" button="inside" placeholder="请输入商品搜索" 
	...
	export default {
		...
		methods: {
			...
			doSearch(keyword) {
				...
				uni.navigateTo({
					url: "clothes-list?keyword="+keyword
				})
			},
			//监听输入
			inputChange(event) {
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['裤子', '套衫', '连衣裙', '外套', '凉鞋', '衬衫', '短袖', '运动鞋', '包', '短靴'];
			},
			//清除历史搜索
			oldDelete() {
				uni.showModal({
